...
namespace RetroFootballManager.ViewModels
{
    ...
    public partial class MatchDayViewModel : BaseViewModel
    {
            ...
            try
            {
                while (!_match.IsFinished)
                {
                    ...
                    if (_match.Phase == MatchPhase.HalfTime && !_halfTimeShown)
                    {
                        ...
                        // Team talk first, then the same tactics/lineup/subs panel as before -
                        // confirming or cancelling either both continue into the second half.
                        OpenTeamTalkDialog();
                        continue;
                    }
        ...
        // --- Half-time team talk (shown before the team-management panel) ---
        ...
        {
            var stats = _match?.Result ?? _humanResult;
            if (stats is null || _homeTeam is null || _awayTeam is null)
                return;

            HomeMatchStats = MatchStatsDisplay.From(_homeTeam.Name, stats.MatchStatsHome);
            AwayMatchStats = MatchStatsDisplay.From(_awayTeam.Name, stats.MatchStatsAway);
